...
class DataMargin():
    ...
    def __init__(self, data_dir, vocab_size, key, pretrained_model_name='allenai/longformer-base-4096'):
        
        with open(data_dir+"train_bert_marginal_15.json") as f:
            train_input = json.load(f)

        self.train_input = [ ctx.strip().split(' EOT ') for ctx in train_input]
        random.shuffle(self.train_input)
        
        with open(data_dir+"valid_bert_marginal_15.json") as f:

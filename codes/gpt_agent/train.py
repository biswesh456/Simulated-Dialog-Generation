...
parser.add_argument('--lr', type=float, default=.000002, metavar='LR', help='Learning Rate (default: .00015)')
...
if args.best:
    ...
    best_string = ''
args.load_path = args.model_dir + "checkpoint_hred_user_" + args.load_name + best_string + "_" + str(args.word_size) + "_" + str(args.hidden_size) + "_" + str(args.batch_size) + '.pth.tar'
...
if args.aug_type == 'none':
    data = Data.GPTDataTopic(data_dir = args.data_dir, key = args.key, args = args)
elif args.aug_type == 'augmented':
...
def train(epoch, start=-1):
    input_ids, response_id, adverse_input_ids, adverse_id, response = data.get_batch(start=start)
...
def validate(start):
    input_ids, response_id, adverse_input_ids, adverse_id, response = data.get_batch(start=start, train=False)
...
#         predicted_list = [predicted.lower().strip().split(" ")]
#         total_gts = [[gt.lower().strip().split(" ")]]
#         bleu_score = corpus_bleu(total_gts, total_preds, smoothing_function=SmoothingFunction().method1)*100
...
args.val_step = int(len(data.train_context)/16)
...
for epoch in range(args.num_epochs):
    ...
    for step in range(0, num_steps_per_epoch):
        ...
        if niter%args.val_step==0:
            ...
            vnloss = 0
            nviter = 0
            for v in range(len(data.valid_context)):
                with torch.no_grad():
                    p, n = validate(v)
                    vploss += p
                    vnloss += n
            ...
            t1 = time.time()
            print(epoch, start, "PLOSS : {0:.3f}".format(ploss/niter), "Val Bleu : {0:.3f}".format(vploss/nviter),\

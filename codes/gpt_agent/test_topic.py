...
parser.add_argument('--prediction', type=str, default=None, help='relative address of data with repect to prediction directory')
...
parser.add_argument('--topic', type=str, default=None, help='topic')
...
data = Data.GPTData_test_topic(data_dir = args.data_dir, Type = args.type) 
final_data = {}
...
def evaluate(start, filecr, batch_size=1):
    input_ids, response, query, kb, context, file_name, state = data.get_batch(start=start)
#     print(args.topic, state['hotel'])
    if state[args.topic]:
    ...
    #     print('gold', response)
    #     print('*'*100)
    #     write_to_file(filecr, predicted, response, query, kb, context)
...
with open(args.prediction_dir + args.prediction + '_' + args.topic +'.json', 'w') as f:
